...
public class ProductRepository : IProductRepository
{
    ...
    {
        //Validar que el name no sea nulo ni sea espacio y que la cantiddd sea mayor a 0
        ...
        //Consulta de EntityFramework
        var product = _db.Products.FirstOrDefault(p => p.Name == name.Trim());
    ...
    //Incluye la categoria de la relación
    ...
    {
        return _db.Products.Include(p => p.Category).OrderBy(p => p.CreationDate).ToList();
    }
    ...
    public bool Save()
    {
        return _db.SaveChanges() >= 0; //Guarda los cambiosen la DB
    }

    public ICollection<Product> SearchProducts(string searchTerm)
    {
        IQueryable<Product> query = _db.Products; //IQueryable: construye la consulta para que se ejecute en la base de datos
